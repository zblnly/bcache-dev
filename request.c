...
static inline struct search *search_alloc(struct bio *bio, struct bcache_device *d)
{
	...
	s->iop.c		= d->c;
	s->iop.bio		= NULL;
	s->iop.inode		= KEY_INODE(&d->inode.k);
...
static void cached_dev_read_done(struct closure *cl)
{
	...
	if (verify(dc, &s->bio.bio) && !s->read_dirty_data)
	...
	if (!test_bit(CACHE_SET_STOPPING, &s->iop.c->flags))
...
static void cached_dev_read_done_bh(struct closure *cl)
{
	...
	bch_mark_cache_accounting(s->iop.c, s->d, !s->cache_miss, s->iop.bypass);
...
static void flash_dev_make_request(struct request_queue *q, struct bio *bio)
{
	...
	struct closure *cl;
	struct bcache_device *d = bio->bi_bdev->bd_disk->private_data;
	unsigned inode = KEY_INODE(&d->inode.k);
	int cpu, rw = bio_data_dir(bio);
